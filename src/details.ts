...
export const frameworks: Record<FrameworkName, Framework> = {
  ...
  gridsome: {
    ...
    initCommend: ['npm install --global @gridsome/cli', 'gridsome create my-app']
  },
  express: {
    name: 'Express',
...
function getVersion(version: string | undefined, updatedAt: any): string | undefined {
  return version ? `${version} (${formatDistance(parseISO(updatedAt[version]), new Date(), { addSuffix: true })})` : undefined
}

export async function getDetails(framework: FrameworkName) {
  const spinner = ora('Loading Details').start();

  let { repo, pkg, publishedAt, author, website, initCommend } = frameworks[framework];
